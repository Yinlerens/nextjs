...
const OverNext30Chart = () => {
  ...
  let data = {
    unit: ['访问量'],
    data: new Array(30).fill('').map(() => randomNum(1, 20000))
  };
  const option = {
    tooltip: {
      trigger: 'axis',
      ...
      formatter: params => {
        ...
        let html = `<div class="box-border flex items-center w-[180px] h-[60px] pl-[20px] bg-[url('../public/images/line-bg.png')] bg-no-repeat bg-[length:100%_100%]">
                        <span class="text-xs text-[#ffffffcc]">${tipData.name} <i class="not-italic text-[#f5b348]">${tipData.value}</i> 人次访问</span>
                    </div>`;
        return html;
      },
  ...
  return <div className="w-full h-full" ref={echartsRef}></div>;
};
